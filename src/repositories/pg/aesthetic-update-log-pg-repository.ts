...
export class PgAestheticUpdateLogRepository extends PgBaseRepository<Record<string, unknown>> {
  constructor(pool: Pool, client?: PoolClient) {
    super(pool, nrm("aesthetic_update_logs"), client);
  }
  ...
  async cleanupZombieTasks(): Promise<void> {
    await this.queryClient.query(
      `UPDATE ${this.tableName} SET status = 'error', error_message = '服务中断，任务未完成（超时清理）', finished_at = NOW() WHERE status = 'running' AND started_at < NOW() - INTERVAL '30 minutes'`,
    );
  }
  ...
  /** 插入运行记录（running 状态） */
  async insertLog(params: { triggerType: string; ageRange: string }): Promise<string> {
    const result = await this.queryClient.query<{ id: string }>(
      `INSERT INTO ${this.tableName} (trigger_type, status, age_range)
       VALUES ($1, 'running', $2)
       RETURNING id`,
      [params.triggerType, params.ageRange],
    );
    ...
    await this.queryClient.query(
      `UPDATE ${this.tableName}
       SET status = $1,
           ...
           finished_at = NOW()
       WHERE id = $9`,
      [status, data.xiaohongshuCount, data.instagramCount, data.weiboCount, data.douyinCount, data.featuresUpdated, data.durationMs, data.errorMessage || null, logId],
    );
  }
  ...
  async findPaginatedLogs(params: {
    ...
    const listResult = await this.queryClient.query(
      `SELECT id, trigger_type, status, age_range,
              xiaohongshu_count, instagram_count, weibo_count, douyin_count, features_updated,
