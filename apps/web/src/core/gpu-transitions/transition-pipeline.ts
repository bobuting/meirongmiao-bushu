...
export class TransitionPipeline {
  ...
  private constructor(device: GPUDevice) {
    this.device = device
    // 使用浏览器首选格式，避免额外的格式转换开销
    // navigator.gpu.getPreferredCanvasFormat() 返回 'bgra8unorm' 或 'rgba8unorm'
    const nav = navigator as Navigator & { gpu?: { getPreferredCanvasFormat?: () => GPUTextureFormat } }
    this.format = nav.gpu?.getPreferredCanvasFormat?.() ?? 'bgra8unorm'
